...
void initTXPin() {
    // DEBUG_TX_PORT_PIN is active LOW, so set it to HIGH initially
    SERIAL_OUT_TX_PORT |= _BV(SERIAL_OUT_TX_PORT_PIN);
    // set pin direction to output
    SERIAL_OUT_TX_DDR |= _BV(SERIAL_OUT_TX_PORT_PIN);
}
...
void write1Start8Data1StopNoParity_C_Version(uint8_t aValue) {
     ...
     */
// start bit
    SERIAL_OUT_TX_PORT &= ~(1 << SERIAL_OUT_TX_PORT_PIN);
    _NOP();
